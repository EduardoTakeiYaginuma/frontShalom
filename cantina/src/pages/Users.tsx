...
const Users: React.FC = (): JSX.Element => {
  ...
  const [sortKey, setSortKey] = useState<
    "nome" | "saldo" | "tipo_usuario" | "quarto"
  >("nome");
  ...
  const filtered = useMemo(() => {
    ...
    return result.sort((a, b) => {
      ...
      switch (sortKey) {
        case "nome":
          compare = `${a.nome} ${a.sobrenome}`.localeCompare(
            `${b.nome} ${b.sobrenome}`,
          );
          break;
        case "saldo":
          ...
          break;
        case "tipo_usuario":
          compare = (permissoes[a.tipo_usuario] || "").localeCompare(
            permissoes[b.tipo_usuario] || "",
          );
          break;
        case "quarto":
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((u, i) => (
                    ...
                    >
                      ...
                      <td className="text-center py-4 px-6">
                        {permissoes[u.tipo_usuario] || "Desconhecido"}
                      </td>
                      <td className="text-center py-4 px-6">{u.quarto}</td>
                      <td className="text-center py-4 px-6 font-semibold text-[#00AB4F]">
                        R$ {u.saldo.toFixed(2)}
                      </td>
                      <td className="text-center py-4 px-6">
                        <button
